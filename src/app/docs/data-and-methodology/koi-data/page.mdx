...
Access to relevant, trusted data is often the hardest part of impact assessment. Koi aggregates and derives lifecycle and market data from authoritative sources, removing a substantial share of the manual work that has slowed the field. We've modeled thousands of proposed climate solutions and provide both the off-the-shelf models and data in-app.

**Data sourcing:** This page contains a representative overview of the data sources used in Koi models, organized by category and use case. We prioritize open-access, trusted datasets, such as IEA and government databases, and often supplement them with bottom-up lifecycle GHG data for solution modeling, value chain segmentation, and boundary definition. Because sources vary by technology, industry, and region, the complete model-specific citations are documented on each model’s Datasheet page (see screenshot below).
...
These sources underpin both high-resolution models for well-characterized technologies and early-stage assessments for emerging ones. Academic and industry journals supply the depth needed for sensitivity analyses across plausible performance ranges and provide otherwise hard-to-obtain evidence on new technologies. Together they offer technical specifications, performance parameters, and emissions factors that enable comprehensive impact modeling.
...
<InternalLink
  ...
  linkText="Life Cycle Analysis (LCA)" /> data are critical in establishing absolute emissions intensities for critical process phases and value chain components.
</Info>
