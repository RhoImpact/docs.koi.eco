...
Koi's **Search** is designed to help you quickly identify the most relevant climate solutions, models, or technologies for any given query or context. We currently support three main search approaches:

1. **Company Search** – Quickly identify the nearest matching Koi model to a company's technology based on the information on a company's website.
2. **Model Search** – Enter text queries or descriptions, and let our semantic vector search find the closest matches among Koi’s climate solution library.
3. **Baseline Search** – Easily locate existing baseline scenarios (e.g., incumbent technologies or processes) for comparisons within your climate impact models.
...
- **Pro Tip**
If you want to view a number of baselines together, you can add multiple baselines to a collection from the technology model page and view them together. See our documentation on Collections for more information.

---

## Frequently Asked Questions

1. **Is domain search the same as model search?**
   - Not exactly. Domain Search specifically scrapes and interprets a given domain’s content in real time. Model Search uses semantic embeddings to match your text query to relevant climate solutions already in our library. Both use vector-based technology, but Domain Search first collects fresh data from the company’s website.
